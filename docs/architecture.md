...
```
source text
  → scanner ─ tokens ─ Pratt parser          → AST            [syntax]
...
- **[syntax](syntax/parsing.md)** — a streaming scanner and a Pratt/precedence-climbing parser build an arena-allocated AST. Nix's awkward corners (interpolation, indented strings, paths, `inherit`, attr patterns, dynamic attrs) are handled here → [nix-syntax](syntax/nix-syntax.md).
...
fix (core) ── bytecode · compiler · vm · eval · jit · probe
              a real dependency cycle (chunk↔jit, force↔compiler, worker↔vm)
              → stays one module
```

The clean-cut modules expose a single facade file each; reaching into their internals is a lint error (it also silently duplicate-compiles). The coupled engine can't be cut without breaking the cycle, so it lives together and is organised by directory instead. See [build](build.md).
...
- **The interpreter is canonical.** The [JIT](jit.md) and [GC](gc.md) are experimental, opt-in (`-Djit`/`-Dtjit`/`-Dgc`), off by default, and never change output. The tracing JIT is *measured-dead* on this workload; the GC works at `--workers=1` but is net-negative on time. They are documented for whoever picks them up.
...
6. **Performance & experiments:** [perf/model](perf/model.md) → [perf/probes](perf/probes.md); [jit](jit.md); [gc](gc.md).
