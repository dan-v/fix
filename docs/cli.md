...
`src/main.zig` is composition only (~150 lines): it dispatches subcommands, then parses options and drives an `Evaluator`. `src/cli/args.zig` owns the option grammar. Everything a developer needs to disassemble bytecode, snapshot the heap, replay execution, and pinpoint a parallelism divergence lives here.
...
| `--print-sched-stats` | after eval, print scheduler/probe/JIT diagnostics (per-worker wall/idle/run, plus any `-Dprof-*` / `-Dtjit` reports) |
...
Use these to isolate whether a wrong parallel result (or a hang) comes from speculation vs fan-out: rerun with each disabled and see which one restores correctness.
...
The runtime trace flags above only do anything on a build compiled with the matching `-D` probe (`-Dvm-trace`, `-Dthunks-log`, `-Dtimeline`); the `-Dprof-*`, `-Dtjit`, `-Dgc` reports surface through `--print-sched-stats`. All `-D` machinery is build-time — exercising a probe means a rebuild. See [perf/probes.md](perf/probes.md) and [build.md](build.md).
