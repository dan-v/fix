...
- **Interactive** — stdin *and* stdout are a tty and `--bare` was not given. One alternate-screen surface integrates the prompt, a bounded output transcript, and the VM explorer. The prompt keeps the hand-rolled editor: emacs-style editing (C-a/C-e/C-k/C-u/C-w/C-y + kill ring, M-b/M-f word motion), persistent history (`$XDG_STATE_HOME/fix/repl-history`, multiline entries round-trip) with C-r incremental search, Tab completion, bracketed paste, SIGWINCH-aware rendering, and C-z suspend/resume. Escape moves from an empty prompt into the VM detail; `q`/Escape returns to the prompt. Raw mode is restored on **every** exit path — orderly return, panic (SIGABRT), fatal signal — via a saved-termios global + chained signal handlers (`repl/term.zig`). Header and footer bars explicitly paint every terminal column rather than relying on erase-to-EOL background behavior.
...
**Commands** (`:?` shows this table in-repl): `:?`/`:help`, `:q`/`:quit`/`:exit`, `:l`/`:load PATH`, `:r`/`:reload`, `:t`/`:type EXPR`, `:p`/`:print EXPR` (deep-force), `:i`/`:inspect EXPR` (kind, thunk state + backing chunk, closure chunk/arity), `:vm [COMMAND | EXPR]` (`:d`/`:disasm` aliases), `:env`, `:gc`.

**`:vm` — focus and explore VM state.** Every successful evaluation updates a stable focused chunk: a closure or bytecode thunk contributes its backing chunk; other values use the evaluation's entry chunk. `:vm EXPR` evaluates and focuses, plain `:vm` reopens the same focus even when no chunk was newly compiled, and `:vm chunk ID` focuses a registry id directly. The interactive detail has code, cold tables (constants, function arguments and positions, attr names/positions, captures, upvalues), source maps, and references on keys 1–4. The sidebar is the compiler's qualified-name hierarchy, starts collapsed except for the focused path, and bounds each expanded name to 200 child names plus 100 directly-attached chunks. Incoming references scan only when their view is requested; no eager whole-registry reverse graph is built.

At NixOS scale the name hierarchy is projected into compact CSR arrays off the compiler hot path. Registry growth triggers a background rebuild while the current generation remains usable; a completed generation swaps in atomically from the UI thread. The transcript retains only its newest 4 MiB and indexes at most 2,000 trailing lines, so printing or compiling millions of objects does not make the terminal surface grow without bound. Bare mode exposes the same hierarchy without a TUI: `:vm ls [@NAME] [LIMIT]` lists one name level, `:vm chunks [@NAME] [LIMIT]` lists direct chunks, and both default to 40 rows with a hard maximum of 1,000. Nothing available through the panes is required for control; the prompt commands remain the universal interface.
...
`--debugger` forces `--workers=1` and disables speculation so the pause point is deterministic — a single demand fiber, no helper racing ahead of the break. The console reads commands from stdin and writes to stderr, so a redirected stdout still receives only the final value. Under the full-screen repl, a pause temporarily yields the alternate screen and restores canonical terminal input; continuing restores raw mode and redraws the integrated surface. Its one-byte nested reader stops debugger buffering from swallowing commands typed ahead for the resumed repl.
