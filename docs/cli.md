...
**`:vm` — focus and explore VM state.** Every successful evaluation updates a stable focused chunk: a closure or bytecode thunk contributes its backing chunk; other values use the evaluation's entry chunk. `:vm EXPR` evaluates and focuses, plain `:vm` reopens the same focus even when no chunk was newly compiled (or opens the collapsed help workspace before the first evaluation), and `:vm chunk ID` focuses a registry id directly. `c`/`t`/`s`/`r` independently show or hide the code, tables (constants, function arguments and positions, attr names/positions, captures, upvalues), source, and references panels; reference rows navigate like links. At 140 columns and wider, real source is placed beside bytecode instead of consuming another tall pane. The sidebar contains collapsed HEAP and BYTECODE roots. BYTECODE retains real binding paths plus synthetic lambda/node segments and projects only the focused chunk and its ancestors through any closed nodes; large explicitly-open sibling sets become recursively expandable range nodes. Scrolled descendants pin the portion of their ancestor chain that would otherwise disappear. Opening references starts one compact asynchronous whole-registry graph build; page refreshes then use constant-time CSR slices instead of rescanning every chunk.
...
Bare mode exposes the same model with fixed-size, composable queries: `:vm ls [@NAME] [LIMIT]` walks one prefix-tree level, `:vm chunks [@NAME] [LIMIT]` lists chunks directly attached to that prefix, `:vm objects [START] [LIMIT]` pages through live object IDs, and `:vm object ID` describes one object and its references. Listings default to 40 rows with a hard maximum of 1,000; `:vm heap` prints aggregate store and object-state counts. Nothing available through the panes is required for control; the prompt commands remain the universal interface.
...
`--debugger` forces `--workers=1` and disables speculation so the pause point is deterministic — a single demand fiber, no helper racing ahead of the break. An interactive REPL uses a full-screen debugger with live stack, source/disassembly, and lazy locals/output panes. It owns an alternate screen when entered from the ordinary inline prompt, but borrows the already-active screen and raw mode when entered from `:vm`; the VM explorer redraws as soon as evaluation resumes. `fix eval --debugger`, a non-TTY REPL, and `fix repl --bare` retain the line-oriented console, which reads stdin and writes stderr so redirected stdout receives only the final value and automation sees no escape sequences.
...
**Source, bytecode & color.** The TUI keeps the selected frame's source centered on its current span and can switch that pane to annotated bytecode without leaving the pause. Its locals pane shows value kinds without forcing thunks; only an explicit expression or `value` command renders and therefore may force a value. The console prints the current line with context and a caret under the span. Source comes from the FileCache (imported files) or the stashed entry text. Rendered values use the normal evaluator palette whenever the terminal takes color; semantic TUI roles are quantized through the shared terminal-color model.
