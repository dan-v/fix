...
**`:vm` — focus and explore VM state.** Every successful evaluation updates a stable focused chunk: a closure or bytecode thunk contributes its backing chunk; other values use the evaluation's entry chunk. `:vm EXPR` evaluates and focuses, plain `:vm` reopens the same focus even when no chunk was newly compiled, and `:vm chunk ID` focuses a registry id directly. The detail document is made of collapsible code, tables (constants, function arguments and positions, attr names/positions, captures, upvalues), source, and references sections; reference rows navigate like links. At 140 columns and wider, real source is placed beside bytecode instead of consuming another tall pane. The sidebar contains collapsed HEAP and BYTECODE roots. BYTECODE expands only the focused chunk's qualified-name ancestor path; large sibling sets become recursively expandable range nodes, and scrolled descendants pin the portion of their ancestor chain that would otherwise disappear. Incoming references scan only when their section is opened; no eager whole-registry reverse graph is built.

At NixOS scale the name hierarchy is projected into compact CSR arrays off the compiler hot path. Registry growth triggers a background rebuild while the current generation remains usable; a completed generation swaps in from the UI thread before the next evaluation mutates the registry. The heap tree's store reservation counts are constant-time; its object-variant/thunk-state census begins asynchronously only when a heap page is opened. The transcript retains only its newest 4 MiB and indexes at most 2,000 trailing lines, so printing or compiling millions of objects does not make the terminal surface grow without bound. Bare mode exposes the same model with fixed-size output: `:vm ls [@NAME] [LIMIT]` lists one name level, `:vm chunks [@NAME] [LIMIT]` lists direct chunks, and both default to 40 rows with a hard maximum of 1,000; `:vm heap` prints aggregate store and object-state counts rather than individual objects. Nothing available through the panes is required for control; the prompt commands remain the universal interface.
...
`--debugger` forces `--workers=1` and disables speculation so the pause point is deterministic — a single demand fiber, no helper racing ahead of the break. The console reads commands from stdin and writes to stderr, so a redirected stdout still receives only the final value. When reached from the VM workspace, a pause temporarily yields the alternate screen and restores canonical terminal input; continuing restores raw mode and redraws the workspace. Its one-byte nested reader stops debugger buffering from swallowing commands typed ahead for the resumed repl.
