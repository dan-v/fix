...
The REPL also offers `:debug EXPR` / `:d EXPR` without requiring a persistent `--debugger` session. It installs the console for that evaluation, gates speculative and fan-out submissions through an atomic scheduler switch, and pauses on a synthetic `builtins.break null` before `EXPR` is forced. Continuing evaluates, binds, and prints the result normally; afterward the UI and scheduler gate are detached, so later REPL evaluations regain their configured parallel behavior.
