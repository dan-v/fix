...
Before each token the scanner skips layout — whitespace, `#` line comments, and `/* */` block comments — so comments never reach the parser. Two byte-run hot loops (layout runs and identifier runs) use SIMD block scanning when the target has vectors, falling back to scalar loops otherwise; character classification is a branchless table lookup (`class_table`) rather than a chain of range comparisons. Keyword recognition dispatches on identifier length (a small jump table) and compares the few candidates of that length, so ordinary identifiers fall straight through.
...
Strings and paths are where the scanner stops being trivial. Their extent cannot be found by a simple character class because `"..."` and `''...''` embed arbitrary nested `${ expr }` interpolations. The scanner delegates extent-finding to [`string_syntax`](nix-syntax.md), which tracks brace depth through nested interpolations (and through nested strings/comments inside them) and returns where the literal ends; the scanner emits one `string`/`path`/`search_path` token spanning the whole thing. Decoding the literal's parts happens later, on demand, not during scanning.
...
Two comptime transforms in `grammar.zig` shape the tables. Unit `pass` productions (`A -> B`, identity action) are eliminated by inlining each non-unit rule into every nonterminal that reaches it through unit rules, so the driver never performs a do-nothing chain reduction — every reduce runs a real action. The production *order* is part of the contract: the driver indexes `act_of_prod` by production number to pick the action.
...
The stacks start at a depth that covers any sane nesting — left recursion keeps lists flat, so depth tracks *nesting* only — and grow geometrically for pathological inputs.
