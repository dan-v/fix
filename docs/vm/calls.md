...
| `tail_call` | tail | closure callee **reuses the current frame** (`replaceCurrentFrame`); other callees behave like `call` followed by `ret`. |
...
| `tail_call_n` | tail | saturated closure reuses the frame (`replaceCurrentFrameMulti`) — deep multi-arg tail recursion does not grow the stack; else folds to a value the following `ret` returns. |
...
- **`strict_param`** (single-param chunk whose body must-forces its parameter). At an `apply_arg` site the VM checks the runtime callee: if it is such a closure, the argument expression is **evaluated eagerly to a value — no thunk**; otherwise it thunks exactly like `thunk_captures`. This catches dynamically-dispatched strict calls the compiler cannot resolve statically.
- **`strict_params`** (bitmask, uncurried chunks). Before a **saturated** `call_n`/`tail_call_n` pushes the frame, `forceStrictArgs` forces the must-force arg positions in place on the stack. Value-preserving (the body forces them anyway); it recovers the eager-arg win for the multi-param case and kills lazy-thunk buildup.
