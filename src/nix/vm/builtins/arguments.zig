...
pub fn appendStringAttr(self: anytype, entries: *std.ArrayListUnmanaged(heap_mod.AttrEntry), name: []const u8, value: []const u8) !void {
...
pub fn dupPathAttr(self: anytype, attrs_id: ObjectId, name: []const u8) ![]u8 {
...
pub fn optionalStringAttr(self: anytype, attrs_id: ObjectId, name: []const u8) !?[]u8 {
    const name_id = try self.intern.intern(name);
...
pub fn requiredStringAttr(self: anytype, attrs_id: ObjectId, name: []const u8) ![]u8 {
    return try optionalStringAttr(self, attrs_id, name) orelse error.MissingAttribute;
}

pub fn optionalBoolAttr(self: anytype, attrs_id: ObjectId, name: []const u8) !?bool {
    const name_id = try self.intern.intern(name);
    const value = self.heap.getAttrValue(attrs_id, name_id) catch |err| switch (err) {
        error.MissingAttribute => return null,
        else => return err,
