...
pub fn tagName(t: Tag) [:0]const u8 {
    return switch (t) {
        .objects => "fix:objects",
        .values => "fix:values",
        .attrs => "fix:attrs",
...
/// Blocks freed back to the cache's park list revert to `.bigblock`.
pub threadlocal var alloc_tag: Tag = .bigblock;
...
test "register / residency / retag / unregister round-trip" {
    ...
    registerRegion(mem.ptr, len, .fiber_stack);
    // Touch two pages; residency must see at least those.
    mem[0] = 1;
    mem[std.heap.pageSize()] = 1;
    var r = residency();
    const fs = @intFromEnum(Tag.fiber_stack);
    ...
    retagRegion(mem.ptr, .bigblock);
    r = residency();
    try std.testing.expectEqual(before.regions[fs], r.regions[fs]);

    unregisterRegion(mem.ptr);
    r = residency();
    const bb = @intFromEnum(Tag.bigblock);
