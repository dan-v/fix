//! NaN-boxed 8-byte Value (selected with `-Dvalue8=true`).
//!
//! Layout (64 bits):
//!
...
pub const Value = extern struct {
    ...
    /// Inline integer encoding. The measurement on real Nix workloads
    /// (see `Heap.Stats.int_buckets`) showed essentially no integers
    /// outside i48 — but a debug-build assertion catches the rare case
    /// during development so it can be addressed via a boxed-int Object
    /// variant before release. Release builds silently truncate.
    ...
    pub fn isFloat_(self: Value) bool {
        return self.isFloat();
    }
    ...
    //
    // Matches Value16's behaviour: scalars + objects compare by tag +
    // payload (which for Value8 is the same as `bits == bits`), and
    // floats compare via IEEE equality (so NaN != NaN even when both
    // are the canonical NaN bit pattern, matching Value16).
...
test "value8: tagged int round-trip" {
...
test "value8: float round-trip and NaN scrub" {
...
test "value8: object id and intern id round-trip" {
...
test "value8: idEq matches Value16 semantics for scalars and objects" {
    ...
    // Floats: NaN != NaN to match Value16's `asFloat() == asFloat()`.
