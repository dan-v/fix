...
test "grisu2 matches nlohmann oracle over 50k doubles" {
    // Differential corpus: 19 curated edge cases + 50k random doubles, each a
    // line `<hex-bits> <nlohmann-output>` split across two @embedFile'd files
    // (generated from nlohmann/json 3.12.0, which is what Nix serializes with).
