...
pub fn spinnerFrame(elapsed_ns: i96) u8 {
    const frames = [_]u8{ '|', '/', '-', '\\' };
    const tick: usize = @intCast(@divTrunc(@max(elapsed_ns, 0), 250 * std.time.ns_per_ms));
    return frames[tick % frames.len];
}

pub fn percentTenths(completed: usize, total: usize) usize {
    if (total == 0) return 1000;
    return completed * 1000 / total;
}

pub fn permille(part: usize, whole: usize) usize {
    if (whole == 0) return 0;
    return part * 1000 / whole;
}

pub fn saturatedSub(lhs: usize, rhs: usize) usize {
    return if (lhs > rhs) lhs - rhs else 0;
}

pub fn writeDuration(writer: *std.Io.Writer, seconds: u64) !void {
    const hours = seconds / 3600;
    const minutes = (seconds / 60) % 60;
    const secs = seconds % 60;
    if (hours != 0) {
        try writer.print("{}h{}m", .{ hours, minutes });
    } else if (minutes != 0) {
        try writer.print("{}m{}s", .{ minutes, secs });
    } else {
        try writer.print("{}s", .{secs});
    }
}

pub fn printDuration(seconds: u64) void {
    const hours = seconds / 3600;
    const minutes = (seconds / 60) % 60;
    const secs = seconds % 60;
    if (hours != 0) {
...
pub fn writeProgressBar(
    ...
    const filled = if (total == 0) width else @min(width, completed * width / total);
    const fill = styleCode(use_color, fill_style);
    const dim = styleCode(use_color, .dim);
    const reset_text = resetCode(use_color);

    try writer.writeByte('[');
    var i: usize = 0;
    while (i < width) : (i += 1) {
        if (i == 0 and filled != 0) try writer.writeAll(fill);
        if (i == filled and filled != width) try writer.print("{s}{s}", .{ reset_text, dim });
        try writer.writeByte(if (i < filled) '#' else '-');
    }
    try writer.print("{s}]", .{reset_text});
}
...
pub fn printProgressBar(
    ...
    var i: usize = 0;
    while (i < width) : (i += 1) {
        if (i == 0 and filled != 0) std.debug.print("{s}", .{fill});
        if (i == filled and filled != width) std.debug.print("{s}{s}", .{ reset_text, dim });
        std.debug.print("{c}", .{if (i < filled) @as(u8, '#') else @as(u8, '-')});
    }
    std.debug.print("{s}]", .{reset_text});
}
