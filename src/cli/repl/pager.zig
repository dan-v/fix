...
const Pager = struct {
        ...
        switch (kind) {
            ...
            .help => {
                const help_text =
                    ...
                    \\  q               leave the browser
                    \\
                    \\Every `chunk #N` in a listing is a link: the compiled
                    \\program is a graph — browse it like pages.
                ;
                return .{
    ...
    /// Every line (except the page's own header) that mentions `chunk #N`
    /// becomes a link to N.
    fn scanLinks(arena: std.mem.Allocator, lines: [][]u8, self_id: ChunkId) ![]Page.Link {
        var links: std.ArrayListUnmanaged(Page.Link) = .empty;
        for (lines, 0..) |line, i| {
            const idx = std.mem.indexOf(u8, line, "chunk #") orelse continue;
            const digits_start = idx + "chunk #".len;
            var end = digits_start;
            while (end < line.len and std.ascii.isDigit(line[end])) end += 1;
            if (end == digits_start) continue;
            const id = std.fmt.parseInt(ChunkId, line[digits_start..end], 10) catch continue;
...
test "scanLinks links chunk mentions but not the header" {
    ...
    try lines.append(a, try a.dupe(u8, "  0005  add_int"));
